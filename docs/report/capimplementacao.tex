...
O método \textbf{addUserTo} guarda o utilizador autenticado como atributo do pedido HTTP, permitindo que o \textbf{resolveArgument} o recupere posteriormente através de \textbf{getUserFrom}. Este método é utilizado pelo \textbf{AuthenticationInterceptor} e pelo \textbf{BearerTokenAuthFilter} para disponibilizar o utilizador autenticado ao resolver. \\
...
A classe \textbf{BearerTokenAuthFilter} é um filtro que é responsável por autenticar o utilizador em cada pedido HTTP e por estabelecer o contexto de segurança da aplicação. Ele extende a classe do Spring \textbf{OncePerRequestFilter}, garantindo assim que a sua execução ocorre apenas uma única vez por pedido. \\
...
O objetivo desta classe é o tratamento de exceções lançadas por erros não tratados. Ao utilizar a anotação \textbf{@RestControllerAdvice} do Spring Boot centralizamos o tratamento de erros não tratados em qualquer ponto da aplicação, garantindo uma resposta consistente para o cliente em caso de erro. \\
...
A arquitetura do cliente web foi desenvolvida recorrendo à biblioteca \textit{React}, com um foco na modularidade e reatividade da interface de utilizador. \\
...
O componente RequireAuthentication tem como responsabilidade intercetaro os acessos a qualquer URI da aplicação que exija que o utilizador tenha sessão iniciada para que a presença de uma sessão ativa possa ser validada antes de os componentes serem exibidos. Ao tentar aceder a uma rota protegida, caso o utilizador não possua um token válido, este será automaticamente redirecionado para a página de login. \\

É de realçar alguns cuidados que foram tomados de forma a que a experiência do utilizador seja o mais fluída e intuitiva possível, nomeadamente o uso do hook \textbf{useLocation}, para que a localização do utilizador seja guardada para que assim possa ser redirecionado para a rota protegida que pretendia aceder após o login bem sucedido e o troço de código "replace=\{true\}", que impede que um utilizador possa usar a função de retroceder do seu browser para voltar para a página anterior após ter sido redirecionado para a página de login. \\


\subsubsection{ThemeProvider}

O \textbf{ThemeProvider} é o componente responsável por gerir e persistir o esquema de cores da aplicação, contanto com dois temas: escuro e cladro. \\

É garantida uma experiência visual consistente ao longo de toda a Single Page Application. Para evitar o efeito visual desagradável de possivelmente carregar inicialmente o tema claro e só depois ser carregado o escuro que um utilizador tenha ativado ao mudar de página, o estado inicial do tema é determinado através de uma função de iniciação lazy no \textbf{useState}, que verifica qual o tema selecionado pelo utilizador, armazenado no localStorage. \\
...
A lógica de implementação desta funcionalidade está centrada no componente \textbf{I18nProvider}, que recorre à \textit{Context API} do React para armazenar o idioma ativo (Locale) e expor o seu dicionário de traduções. \\
