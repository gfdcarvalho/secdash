...
class SastControllerTests: ControllerTestsBase() {

    @BeforeEach
    fun setup() {
        jdbi.useHandle<Exception> { handle ->
            handle.execute("select init()")
            handle.execute("select test_data_for_SastControllerTests()")
        }
    }
    ...
    @Test
    fun `get sast alert should return 200 ok with alert`() {
        val token = login("testUsername1", "testpassword1")
        val sastAlertId = 1
        client().get()
            .uri("/sast/$sastAlertId")
            .header("Authorization", "Bearer $token")
            .exchange()
            .expectStatus().isOk
            .expectBody<SastAlertDetail>()
    }

    @Test
    fun `get sast alert that does not exist should return 404`() {
        val token = login("testUsername1", "testpassword1")
        val sastAlertId = 999
        client().get()
            .uri("/sast/$sastAlertId")
            .header("Authorization", "Bearer $token")
            .exchange()
            .expectProblem(HttpStatus.NOT_FOUND, Problem.notFound)
    }

    @Test
    fun `get sast alert with user that does not have access to that alert should return 403`() {
